...
from .command          import LinkCommand, LinkCommandGenerator
...
class HeaderPacketReceiver(Elaboratable):
    ...
    header packet reception, buffering, and flow control (credit management).
    ...
    def __init__(self, *, buffer_count=4, downstream_facing=False):
        ...
        # Header Packet Queue
        self.packet_pending        = Signal()
        self.packet                = HeaderPacket()
        self.consume_packet        = Signal()
    ...
    def elaborate(self, platform):
        ...
        m.d.comb += [
            ...
            self.packet_pending  .eq(buffers_filled > 0),

            # Always provide the value of our oldest packet out to our consumer.
            self.packet          .eq(buffers[read_pointer])
        ]
        ...
        with m.If(self.packet_pending & self.consume_packet):
